...
const timings = {
  ...
  routeTimeout: Math.max(15000, Number(routing['route-timeout'] || 25) * 1000),
...
const state = {
  phase: 'starting', bot: null, controller: null, cycle: null, reconnectTimer: null,
  stopping: false, connectedAt: null, connectingSince: null, nextReconnectAt: null,
  lastPacketAt: null, lastError: null, lastKickReason: null, lastDisconnectReason: null,
  lastDnsError: null, activeRoute: null, availableRoutes: [], remoteEndpoint: null,
  connectionCycles: 0, connectionAttempts: 0, successfulJoins: 0, disconnects: 0,
  actionCounts: { move: 0, jump: 0, crouch: 0, punch: 0 },
  statusProbeFailures: 0, statusProbeInFlight: false, lastStatusProbeAt: null,
  lastStatusSuccessAt: null, lastStatusError: null, lastStatusLatencyMs: null,
  lastStatusVersion: null, lastStatusPlayersOnline: null, lastStatusDescription: null
};

function validConfig() {
  return Boolean(config.host && config.username && Number.isInteger(config.port) && config.port > 0 && config.port <= 65535);
}
...
const usableSocket = (activeBot = state.bot) => Boolean(activeBot?._client?.socket && !activeBot._client.socket.destroyed && activeBot._client.socket.writable);
const packetsAreFresh = () => Boolean(state.lastPacketAt && Date.now() - state.lastPacketAt <= timings.packetSilence);
...
function statusIsFresh() {
  ...
  if (!state.lastStatusSuccessAt) return connectedFor <= timings.statusStartupGrace + timings.statusInterval;
  const window = timings.statusInterval * (maxStatusFailures + 1);
  return Date.now() - state.lastStatusSuccessAt <= window && state.statusProbeFailures < maxStatusFailures;
}

function healthy() {
  return Boolean(state.phase === 'online' && state.bot?.player && usableSocket() && packetsAreFresh() && statusIsFresh());
}

function healthPayload() {
  const now = Date.now();
  return {
    service: 'Minecraft AFK bot', healthy: healthy(), connected: healthy(),
    connecting: ['resolving', 'connecting', 'routing-fallback'].includes(state.phase),
    phase: state.phase, username: config.username, server: `${config.host}:${config.port}`,
    clientVersion: config.version || 'auto', auth: config.auth, activeRoute: state.activeRoute,
    availableRoutes: state.availableRoutes, remoteEndpoint: state.remoteEndpoint,
    ...
    connectionCycles: state.connectionCycles, connectionAttempts: state.connectionAttempts,
    successfulJoins: state.successfulJoins, disconnects: state.disconnects,
    lastDisconnectReason: state.lastDisconnectReason, lastKickReason: state.lastKickReason,
    lastError: state.lastError, lastDnsError: state.lastDnsError,
    serverStatus: {
      enabled: statusEnabled, failures: state.statusProbeFailures, maxFailures: maxStatusFailures,
      ...
      lastError: state.lastStatusError, latencyMs: state.lastStatusLatencyMs,
      version: state.lastStatusVersion, playersOnline: state.lastStatusPlayersOnline,
      description: state.lastStatusDescription
    },
    actions: { ...state.actionCounts }, checkedAt: new Date(now).toISOString()
  };
}
...
const query = (resolver, method, value) => timeout(resolver[method](value), timings.dnsTimeout, `${method} ${value}`);

async function buildRoutes() {
  const routes = [];
  const seen = new Set();
  const add = route => {
    if (!route?.host || !Number.isInteger(route.port)) return;
  ...
  for (const item of resolvers) {
    try {
      ...
      for (const address of addresses || []) add({ name: `fresh-a-via-${item.name}`, host: address, port: config.port, fakeHost: config.host });
      if (addresses?.length) {
        console.log(`[DNS] ${item.name} resolved ${config.host} to ${addresses.join(', ')}.`);
        break;
      }
    } catch (error) { errors.push(`${item.name} A: ${reasonText(error)}`); }
  }
  ...
  if (routing['srv-fallback'] !== false) {
    ...
    for (const item of resolvers) {
      try {
        ...
        for (const record of records) add({ name: `srv-via-${item.name}`, host: String(record.name).replace(/\.$/, ''), port: Number(record.port), fakeHost: config.host });
        if (records.length) {
          console.log(`[DNS] ${item.name} resolved ${srvName}: ${records.map(record => `${record.name}:${record.port}`).join(', ')}.`);
          break;
        }
      } catch (error) { errors.push(`${item.name} SRV: ${reasonText(error)}`); }
    }
  }
...
function statusResultLooksOnline(result) {
  ...
  if (/server\s+is\s+offline|not\s+online|not\s+running|server\s+offline|starting|stopping|queued|unavailable/.test(combined)) return false;
...
function pingRoute(route) {
  ...
  return minecraftProtocol.ping({
    ...
    connect: client => client.setSocket(net.connect({ host: route.host, port: route.port }))
  }).then(result => ({ result, latencyMs: Number(result?.latency) || Date.now() - started, route }));
}
...
async function probeServerStatus(routes) {
  ...
  for (const route of unique) {
    try {
      const response = await pingRoute(route);
      if (!statusResultLooksOnline(response.result)) throw new Error(`status reports offline: ${descriptionText(response.result?.description) || 'no description'}`);
      return response;
    } catch (error) { errors.push(`${routeText(route)}: ${reasonText(error)}`); }
  }
  throw new Error(errors.join(' | ') || 'no status routes available');
}
...
function scheduleCycle(reason = '') {
  ...
  console.log(`[BOT] New connection cycle in ${Math.round(timings.reconnect / 1000)}s${reason ? ` after ${reason}` : ''}.`);
...
async function startCycle() {
  ...
  try { cycle.routes = await buildRoutes(); }
  catch (error) {
...
function connectRoute(cycle) {
  ...
  if (cycle.index >= cycle.routes.length) {
    ...
    state.activeRoute = null;
    scheduleCycle('all routes failed');
    return;
  }
  ...
  try {
    const options = {
      host: route.host, port: route.port, username: config.username, auth: config.auth,
      version: config.version || false, keepAlive: true,
      checkTimeoutInterval: timings.protocolKeepAlive, respawn: true,
    ...
    setTimeout(() => connectRoute(cycle), 1000).unref?.();
    return;
  }
  ...
  const finish = reason => {
    ...
    if (state.stopping) { state.phase = 'stopped'; return; }
    if (spawned) {
      cycle.active = false;
      state.cycle = null;
      state.activeRoute = null;
  ...
  const runStatusProbe = async () => {
    ...
    try {
      const response = await probeServerStatus([route, ...cycle.routes]);
      if (done || closing) return;
      ...
      if (state.statusProbeFailures >= maxStatusFailures) close(`server status failed ${state.statusProbeFailures} consecutive times: ${state.lastStatusError}`);
    } finally { state.statusProbeInFlight = false; }
  };
  ...
  client?.once('connect', () => {
    ...
    state.remoteEndpoint = socket?.remoteAddress && socket?.remotePort ? `${socket.remoteAddress}:${socket.remotePort}` : `${route.host}:${route.port}`;
    try { socket?.setKeepAlive(true, timings.tcpKeepAlive); socket?.setNoDelay(true); }
    catch (error) { console.log(`[NETWORK] TCP tuning warning: ${reasonText(error)}`); }
    console.log(`[NETWORK] TCP established to ${state.remoteEndpoint} through ${route.name}.`);
  });

  routeTimer = setTimeout(() => {
    if (!spawned && !done) close(`no spawn after ${Math.round(timings.routeTimeout / 1000)}s`);
  ...
  activeBot.once('spawn', () => {
    ...
    state.statusProbeFailures = 0;
    state.lastStatusError = null;
    state.lastStatusSuccessAt = null;
    console.log(`[BOT] Joined successfully through ${routeText(route)}.`);
    console.log(`[WATCHDOG] Packets every ${Math.round(timings.packetCheck / 1000)}s, status every ${Math.round(timings.statusInterval / 1000)}s, ${maxStatusFailures} failed status probes trigger recovery.`);
    ...
    packetTimer = setInterval(() => {
      if (done || closing) return;
      if (!usableSocket(activeBot)) { close('socket closed or not writable'); return; }
    ...
    const messages = Array.isArray(chatConfig.messages) ? chatConfig.messages.filter(message => typeof message === 'string' && message.trim()) : [];
  ...
  activeBot.on('resourcePack', () => {
    try { activeBot.acceptResourcePack?.(); console.log('[BOT] Accepted resource pack request.'); }
    catch (error) { console.log(`[BOT] Resource pack response failed: ${reasonText(error)}`); }
  });
  if (utils['chat-log'] !== false) activeBot.on('chat', (name, message) => console.log(`[CHAT] <${name}> ${message}`));
  ...
  activeBot.on('error', error => {
    ...
    if (!spawned || /ECONN|ETIMEDOUT|EHOSTUNREACH|ENETUNREACH|EPIPE|ENOTFOUND|socket|keepalive|timed out/i.test(message)) close(`network error: ${message}`);
  });
